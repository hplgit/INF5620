...
cp ~/vc/doconce/bundled/html_styles/style_vagrant/template_vagrant.html .
...
doconce replace 'Here goes a footer, if desired, maybe with a Copyright &copy;' 'H. P. Langtangen &copy; 2013' template_vagrant.html
...
doconce format html report2 --html_style=vagrant --html_template=template_vagrant.html
...
mkdir _static
mv -f *.png *.html ._*.html *.pdf sphinx-* css js *.ipynb *.mwiki report.txt _static
mv -f _static/index*.html .  # don't copy the index file
rm -rf index_with_doconce_commands.html.html style_github*  _static/template_vagrant.html .*_file_collection  # unwanted byproducts

# Copy all doconce source files
mkdir doconce_src
cp report.do.txt .publish_references.pub publish_config.py BE.* FE.* CN.* error.* doconce_src
