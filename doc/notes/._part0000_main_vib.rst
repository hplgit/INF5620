...
problem  :eq:`vib:ode1` we follow the `four steps <http://hplgit.github.com/INF5620/doc/notes/decay-sphinx/main_decay.html#the-forward-euler-scheme>`_ in [Ref1]_.
...
(see 
`examples <http://hplgit.github.com/INF5620/doc/notes/decay-sphinx/main_decay.html#compact-operator-notation-for-finite-differences>`_ in [Ref1]_). The difference :eq:`vib:ode1:step3` has the operator
...
for a simple `ODE model <http://tinyurl.com/k3sdbuv/notes/decay-sphinx/main_decay.html#computing-convergence-rates>`_, yields a good method for
...
The code segments are found in the file `vib_empirical_analysis.py <https://github.com/hplgit/INF5620/blob/gh-pages/src/vib/vib_empirical_analysis.py>`_.
...
on the program `vib_plot_freq.py <https://github.com/hplgit/INF5620/blob/gh-pages/src/vib/vib_plot_freq.py>`_).
...
discretization parameter. It is the product :math:`\omega\Delta t` is
...
:eq:`vib:model2:ueq`-:eq:`vib:model2:veq`
becomes
...
The `vib_odespy.py <https://github.com/hplgit/INF5620/blob/gh-pages/src/vib/vib_odespy.py>`_
...
:eq:`vib:model2:EulerCromer:ueq1` and then :math:`v^{n+1}` from
:eq:`vib:model2:EulerCromer:veq1` using the recently computed value
...
.. allows :eq:`vib:model2:EulerCromer:ueq1`


The scheme
:eq:`vib:model2:EulerCromer:ueq1`-:eq:`vib:model2:EulerCromer:veq1`
...
:eq:`vib:model2:EulerCromer:ueq1`-:eq:`vib:model2:EulerCromer:veq1`.
From :eq:`vib:model2:EulerCromer:veq1` we have
:math:`v^n = v^{n-1} - \Delta t \omega^2u^{n}`, which can be inserted
in :eq:`vib:model2:EulerCromer:ueq1` to yield
...
using :eq:`vib:model2:EulerCromer:ueq1`:

.. math::
         v^{n-1} = \frac{u^n - u^{n-1}}{\Delta t},
        

and when this is inserted in :eq:`vib:model2:EulerCromer:elim1` we get
...
:eq:`vib:model2:EulerCromer:ueq1` implies :math:`u^1=u^0`, while
:eq:`vib:model2:EulerCromer:veq1` says :math:`v^1=-\omega^2 u^0`.
...
.. math::
   :label: vib:model2:EulerCromer:ueq1s
...
.. math::
   :label: vib:model2:EulerCromer:veq1s
...
.. math::
   :label: vib:model2:EulerCromer:ueq1s2
...
.. math::
   :label: vib:model2:EulerCromer:veq1s2
...
In the latter case we can eliminate the :math:`v`
...
Using the discretized equation :eq:`vib:model2:EulerCromer:veq1s2` for
:math:`n=0` yields
...
.. math::
         [u'|u'|]^n = u'(t_n+\frac{1}{2})|u'(t_n-\frac{1}{2})|{\thinspace .}

The next step is to approximate
:math:`u'` at :math:`t_{n\pm 1/2}`, but here a centered differences can be used:
...
This results in a `moving window following the function <http://tinyurl.com/k3sdbuv/notes/mov-vib/vib_generalized_dt0.05/index.html>`_ on the screen.
...
schemes <http://hplgit.github.io/INF5620/doc/notes/decay-sphinx/._part0007_main_decay.html#example-adaptive-runge-kutta-methods>`_
...
the section :ref:`vib:model2:ueq`.
...
The program `vib.py <https://github.com/hplgit/INF5620/blob/gh-pages/src/vib/vib.py>`_
...
for an `ODE model <http://hplgit.github.com/INF5620/doc/notes/decay-sphinx/main_decay.html#implementing-simple-problem-and-solver-classes>`_.
