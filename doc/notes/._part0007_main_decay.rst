...
.. math::
   :label: decay:fd2:Dop:tn:fw
        ...
        \lbrack D_t^+ t\rbrack^n = \frac{t_{n+1}-t_n}{\Delta t}=\frac{(n+1)\Delta t - n\Delta t}{\Delta t}=1,\
...
.. math::
   :label: decay:fd2:Dop:tn:bw
        ...
        \lbrack D_t^- t\rbrack^n = \frac{t_{n}-t_{n-1}}{\Delta t}=\frac{n\Delta t - (n-1)\Delta t}{\Delta t}=1,\
...
and :math:`v(t)` (modeling, e.g., the radioactive decay of two substances):
...
.. math::
        ...
        v^{n+1} = v^n + \Delta t (-a_vv^{n+1} + a_uu^{n+1}){\thinspace .}
...
Exercise 11: Experiment with precision in tests and the size of :math:`u`
-------------------------------------------------------------------------
...
Filename: ``decay_AdamBashforth2.py``.
...
large :math:`\Delta t`?
Filename: ``decay_AdamBashforth3.py``.
