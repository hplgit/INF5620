...
if [ $? -ne 0 ]; then echo "doconce could not compile document $filename.do.txt - abort"; exit 1; fi
doconce replace 'P$d$' 'Pd' ${filename}-solarized.html
...
doconce replace 'P$d$' 'Pd' ${filename}-reveal.html
...
doconce replace 'P$d$' 'Pd' ${filename}-deck.html
...
doconce replace 'P$d$' 'Pd' ${filename}-1.html
...
doconce replace 'P$d$' 'Pd' ${filename}.html
