...
approximations are desired. Figure :ref:`fem:motivation:fig:dolfin` shows
...
method, 2) the Galerkin or :math:`L_2` projection method,
...
:math:`(\cdot,\cdot)` for two different things:
:math:`(a,b)` for scalar quantities :math:`a` and :math:`b` means the vector starting in
the origin and ending in the point :math:`(a,b)`, while :math:`(\boldsymbol{u},\boldsymbol{v})` with
vectors :math:`\boldsymbol{u}` and :math:`\boldsymbol{v}` means the inner product of these vectors.
...
Minimizing :math:`||\boldsymbol{e}||^2` implies that :math:`\boldsymbol{e}` is
...
This approach is known as *projection*, because we it is the same as
projecting the vector onto the subspace.
We may also use the term *Galerkin's method*.
...
Since an arbitrary :math:`\boldsymbol{v}\in V` can be expressed as
:math:`s\boldsymbol{\psi}_0`, :math:`s\in\mathbb{R}`,
...
.. math::
        ...
        (\boldsymbol{e}, \boldsymbol{\psi}_0)=0\quad\Leftrightarrow\quad
        (\boldsymbol{f} - c_0\boldsymbol{\psi}_0, \boldsymbol{\psi}_0)=0
        \thinspace .
        

The latter equation gives :eq:`fem:vec:c0` for :math:`c_0`.
...
Now we want to find :math:`c_0,\ldots,c_Nf`, such that :math:`\boldsymbol{u}` is the best
approximation to :math:`\boldsymbol{f}` in the sense that the distance, or error,
...
:math:`c_0,\ldots,c_N` is obtained by setting
...
.. math::
         (\boldsymbol{f} - \sum_{j=0}^N c_j\boldsymbol{\psi}_j, \boldsymbol{\psi}_i) = (\boldsymbol{f}, \boldsymbol{\psi}_i) - \sum_{j\in\mathcal{I}}
...
.. math::
   :label: fem:approx:ufem
        ...
        u = \sum_{j\in\mathcal{I}} c_j{\psi}_j\thinspace .
        
        

The index set :math:`\mathcal{I}` is defined as :math:`\mathcal{I} =\{0,\ldots,N\}`.
For now, in this introduction, we shall look at functions of a
single variable :math:`x`:
:math:`u=u(x)`, :math:`{\psi}_i={\psi}_i(x)`, :math:`i\in\mathcal{I}`. Later, we will extend
...
form :eq:`fem:approx:ufem` can be used in time too).
...
.. math::
   :label: fem:approx:LS:E
        ...
        E = (f(x)-\sum_{j\in\mathcal{I}} c_j{\psi}_j(x), f(x)-\sum_{j\in\mathcal{I}} c_j{\psi}_j(x))\thinspace .
...
We get can rewrite :eq:`fem:approx:LS:E`,
...
.. math::
        ...
        E(c_i, \ldots, c_N) = (f,f) -2\sum_{j\in\mathcal{I}} c_j(f,{\psi}_i)
        + \sum_{p\in\mathcal{I}}\sum_{q\in\mathcal{I}} c_pc_q({\psi}_p,{\psi}_q)\thinspace . 

Minimizing this function of :math:`N+1` scalar variables
:math:`c_i`, :math:`i\in\mathcal{I}`, requires differentiation
with respect to :math:`c_i`, for :math:`i\in\mathcal{I}`. The resulting
...
.. math::
   :label: fem:approx:Galerkin0
        ...
        (e,{\psi}_i)=0,\quad i=\in\mathcal{I}\thinspace .
...
the Galerkin or projection method are equivalent.
...
That is, :math:`{\psi}_0(x)=1`, :math:`{\psi}_1(x)=x`, and :math:`N=1`.
We seek
...
the symbol ``x``), ``psi`` is a list of :math:`{\psi}_i`, :math:`i\in\mathcal{I}`,
and ``Omega`` is a 2-tuple/list holding the domain :math:`\Omega`:
...
The output from ``least_squares`` gives :math:`c_i=0` for :math:`i>2`.
In fact, we have a general result that
if :math:`f\in V`, the least squares and Galerkin/projection methods compute
the exact solution :math:`u=f`.

The proof is straightforward: if :math:`f\in V`, :math:`f` can be expanded in
terms of the basis functions, :math:`f=\sum_{j\in\mathcal{I}} d_j{\psi}_j`, for
some coefficients :math:`d_i`, :math:`i\in\mathcal{I}`,
and the right-hand side then has entries


.. math::
         b_i = (f,{\psi}_i) = \sum_{j\in\mathcal{I}} d_j({\psi}_j, {\psi}_i) = \sum_{j\in\mathcal{I}} d_jA_{i,j}
        \thinspace . 

The linear system :math:`\sum_jA_{i,j}c_j = b_i`, :math:`i\in\mathcal{I}`, is then


.. math::
         \sum_{j\in\mathcal{I}} c_jA_{i,j} = \sum_{j\in\mathcal{I}}d_jA_{i,j},
        \quad i\in\mathcal{I},

which implies that :math:`c_i=d_i` for :math:`i\in\mathcal{I}`.
...
by functions on the form :math:`u(x)=\sum_{j\in\mathcal{I}}c_jx^j`, :math:`N=10`.
Analytically, we know that :math:`c_j=0` for :math:`j>2`, but ill-conditioning
may produce :math:`c_j\neq 0` for :math:`j>2`.
...
Increasing :math:`N` to 12 makes the numerical solver in ``sympy`` report
...
.. math::
         {\psi}_i(x) = \sin ((i+1)\pi x),\quad i\in\mathcal{I}{\thinspace .} 
...
.. math::
        ...
        u(x) = f(0) + \sum_{j\in\mathcal{I}} c_j{\psi}_j(x)
        \thinspace .
        

However, this adjustments introduces a new problem at :math:`x=1` since
...
.. math::
        ...
        u(x) = f(0)(1-x) + xf(1) + \sum_{j\in\mathcal{I}} c_j{\psi}_j(x)
        \thinspace .
...
.. math::
        ...
        c_i = 2\int_0^1 f(x)\sin ((i+1)\pi x) {\, \mathrm{d}x},\quad i\in\mathcal{I},
...
:math:`x_{i}`, :math:`i\in\mathcal{I}`:


.. math::
        
        u(x_{i}) = \sum_{j\in\mathcal{I}} c_j {\psi}_j(x_{i}) = f(x_{i}),
        \quad i\in\mathcal{I}{\thinspace .} 

This criterion also gives a linear system
with :math:`N+1` unknown coefficients :math:`c_i`, :math:`i\in\mathcal{I}`:
...
The method is often referred to as a *collocation method*
and the :math:`x_{i}` points are known as *collocation points*.
Others view the approach as an *interpolation method*
...
In that case the :math:`x_{i}` points are called *interpolation points*.
...
Given :math:`f`  as a ``sympy`` symbolic expression ``f``, :math:`{\psi}_i`, :math:`i\in\mathcal{I}`,
as a list ``psi``, and a set of points :math:`x_i`, :math:`i\in\mathcal{I}`,  as a list or array
``points``, the following Python function sets up and solves the matrix system
for the coefficients :math:`c_i`, :math:`i\in\mathcal{I}`:
...
Note that it is convenient to turn the expressions ``f`` and
``psi`` into Python functions which can be called with
elements of ``points`` as arguments when building the matrix and
the right-hand side.
...
a diagonal matrix: formulas for the coefficients :math:`c_i`, :math:`i\in\mathcal{I}`, can
...
.. math::
   :label: fem:inter:prop
        ...
        {\psi}_i(x_s) = \left\lbrace\begin{array}{ll}
        1, & i=s,\\ 
        0, & i\neq s,
        \end{array}\right.
...
.. math::
        ...
        c_i = f(x_{i}),\quad i\in\mathcal{I},
...
.. math::
        ...
        u(x) = \sum_{j\in\mathcal{I}} f(x_{i}){\psi}_i(x)\thinspace . 
...
in the symbolic and numerical case (think of ``x`` being an
...
and numerical computing of the basis functions and points:
        ...
        >>> psi, points = Lagrange_polynomials_01(x, N=3, symbolic=True)
        >>> points
        [0.0, 0.5, 1.0]
        >>> psi
        [-0.0, 1.0, 0.0]
...
when looking at the individual Lagrange polynomials: Figure :ref:`fem:approx:global:Lagrange:fig:abs:Lag:unif:osc` shows two such polynomials of degree 11, and it is clear
...
using Chebyshev nodes (compared with Figure :ref:`fem:approx:global:Lagrange:fig:abs:Lag:unif:7:14`).
...
:math:`\Omega`, see Figure :ref:`fem:approx:fe:fig:u:sin` for an example. We shall
...
.. figure:: fig-fem/u_example_fe.png
   :width: 600
...
Standard finite element basis functions are denoted by
:math:`{\varphi}_i(x)`. In the current approximation problem we shall
simply take :math:`{\psi}_i = {\varphi}_i`, but later, when solving
differential equations, the functions :math:`{\psi}_i` used in
the approximation of the unknown function may be chosen
as a subset of a larger set of finite element basis functions :math:`{\varphi}_i`.
...
.. math::
        ...
        u(x_{i}) = \sum_{j\in\mathcal{I}} c_j {\psi}_j (x_{i}) =
        \sum_{j\in\mathcal{I}} c_j {\varphi}_j (x_{i}) = c_i {\varphi}_i (x_{i}) = c_i
        \thinspace .
...
Example on quadratic :math:`{\varphi}_i`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Nodes are marked with circles on the :math:`x` axis in the figure, and
...
do not share the node with global node number :math:`i`.
...
Example on linear :math:`{\varphi}_i`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Example on cubic :math:`{\varphi}_i`
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
piecewise linear. Similarly, *quadratic elements* and
...
Consider P1 (piecewise linear) elements. Nodes and elements
numbered consecutively from left to right imply the nodes
:math:`x_i=i h` and the elements
...
provided in Figure :ref:`fem:approx:fe:fig:P1`.  First we clearly see
from Figure :ref:`fem:approx:fe:fig:P1` that the important property
...
The element :math:`A_{i,i-1}` in the coefficient matrix can be calculated as


.. math::
        
        \int_\Omega {\varphi}_i{\varphi}_{i-1}{\, \mathrm{d}x} = \int_{x_{i-1}}^{x_{i}}
        \left(1 - \frac{x - x_{i-1}}{h}\right)\frac{x - x_{i}}{h} {\, \mathrm{d}x} =
        \frac{h}{6}
        \thinspace .
        

It turns out that :math:`A_{i,i+1} =h/6` as well and that
:math:`A_{i,i}=2h/3`. The numbers are modified for :math:`i=0` and :math:`i=N`:
:math:`A_{0,0}=h/3` and :math:`A_{N,N}=h/3`.
...
.. math::
   :label: fem:approx:fe:bi:formula1
        ...
        b_i = \int_{x_{i-1}}^{x_{i}} \frac{x - x_{i-1}}{h} f(x){\, \mathrm{d}x}
...
.. math::
        ...
        r,s\in{\mathcal{I}_d} =\{ 0,1,\ldots,d \},
...
Figure :ref:`fem:approx:fe:fig:assembly` illustrates how element matrices
...
.. _fem:approx:fe:fig:assembly:

.. figure:: fig-fem/matrix-assembly.png
   :width: 600
...
:math:`b_i^{(e)}`, for :math:`i=q(e,r)`, :math:`r\in{\mathcal{I}_d}`, in an *element vector*


.. math::
        
        \tilde b_r^{(e)}=\{ \tilde b_r^{(e)}\},\quad r\in{\mathcal{I}_d}{\thinspace .}
...
through the domain, :math:`x_L=x_{e}` and :math:`x_R=x_{e+1}`.)
Let :math:`X` be the coordinate
...
:math:`d` and :math:`f(x)`. A simple choice is :math:`d=1` and :math:`f(x)=x(1-x)`
...
and higher-degree polynomials that very much more algebra, but ``sympy``
...
`fe_approx1D.py <http://tinyurl.com/jvzzcfn/fem/fe_approx1D.py>`_. Some of the functions are explained below.
...
floating-point value of :math:`{\tilde{\varphi}}_r(X)`. The
``Lagrange_polynomial`` function, copied here
from the section :ref:`fem:approx:global:Fourier`,
works with both symbolic and
numeric ``x`` and ``points`` variables.
...
by the mapping formula such that ``f`` contains the variable ``X``.
...
we can compute the coefficients :math:`c_i`, :math:`i\in\mathcal{I}`, in the expansion
...
solution procedure implied by ``A.LUsolve`` is symbolic,
otherwise, when ``A`` and ``b`` are ``numpy`` arrays, a standard
...
We may, for comparison, compute the ``c`` vector for an interpolation/collocation
method, taking the nodes as collocation points.
This is carried out by evaluating ``f`` numerically at the nodes:
...
When ``symbolic`` is ``False``, :math:`u(x)` can be computed at a (large)
...
number of points in each element, and the discrete :math:`(x,u)` values on
each elements are stored in arrays that are finally
concatenated to form global arrays
with the :math:`x` and :math:`u` coordinates for plotting. The details are
found in the ``u_glob`` function in
``fe_approx1D.py``.
...
It is easier to look at the integration in :math:`x` coordinates, which
...
expression than :eq:`fem:approx:fe:bi:formula1`:


.. math::
        
        b_i = h\left( \frac{1}{2} \phi_i(x_{0})f(x_{0}) +
...
The reason for this simple formula is simply that :math:`\phi_i` is either
...
A matrix whose majority of entries are zeros, are known as a *sparse*
...
and :ref:`fem:approx:fe:sparsity:P3` exemplifies sparsity patterns.
...
.. _fem:approx:fe:fd:colloc:

Collocation or interpolation
----------------------------

Let :math:`x_{i}`, :math:`i\in\mathcal{I}`, be the nodes in the mesh.
Collocation means


.. math::
        
        u(x_{i})=f(x_{i}),\quad i\in\mathcal{I}{\thinspace .}
        

With :math:`u(x)\approx\sum_{j\in\mathcal{I}}c_j{\psi}_j(x)` and :math:`{\psi}_j`
chosen as :math:`{\varphi}_j`, we get


.. math::
         \sum_{j\in\mathcal{I}} c_j {\varphi}_j(x_{i}) = f(x_{i}),

but :math:`{\varphi}_j(x_{i})=0` if :math:`i\neq j` so the sum collapses to one
term :math:`c_i{\varphi}_i(x_{i}) = c_i`, and we have the result
...
That is, :math:`u` *interpolates* :math:`f` at the node points (the values coincide
at these points, but the variation between the points is dictated by
the type of polynomials used in the expansion for :math:`u`).
The collocation/interpolation approach is obviously much simpler and
faster to use than the least squares  or projection/Galerkin approach.

*Remark.* When dealing with approximation of functions via finite elements,
all the three methods are in use, while the least squares and collocation
methods are used to only a small extend when solving differential equations.
...
context will typically just sample :math:`f` at the grid points. That is,
the discrete version of :math:`f(x)` is the set of point values
:math:`f(x_i)`, :math:`i\in\mathcal{I}`, where :math:`x_i` denotes a mesh point.
...
.. math::
        ...
        \sum_{j\in\mathcal{I}} c_j ({\psi}_i,{\psi}_j) = (f,{\psi}_i),\quad i\in\mathcal{I}{\thinspace .}
...
The finite difference counterpart of this equation is just :math:`u_i=f_i`.
...
The left-hand side of :eq:`fem:deq:1D:approx:deq:massmat:diffeq2` can
be manipulated to equal
...
To proceed with the right-hand side, we turn to numerical integration schemes.
Let us say we use the Trapezoidal method for :math:`(f,{\varphi}_i)`, based on
the node points :math:`x_{i}=i h`:
...
Turning to Simpson's rule with sample points also in the middle of
the elements, :math:`x_i=i h/2`, :math:`i=0,\ldots,2N`, it reads in general
...
where :math:`\tilde h = x_i - x_{i-1}= h/2` is the spacing between the sample points.
We see that the midpoints with odd numbers have
the weight :math:`2h/3` while the node points with even numbers have the
weight :math:`h/3`. Since :math:`{\varphi}_i=0` at the even numbers, except
for :math:`x_{2i}=x_{i}`, and :math:`{\varphi}_i=0` at all the midpoints,
on the midpoints and :math:`4h/3` on the node points. Since :math:`{\varphi}_i`
vanishes at all the node points, except :math:`\xi{i}`, and
except :math:`x_{2i-1}=\xi{i}-h/2` and
:math:`x_{2i+1}=\xi{i}+h/2`, where :math:`{\varphi}_i=1/2`, we get


.. math::
        
        (f,{\varphi}_i) \approx \frac{h}{3}(f(x_{i}-\frac{1}{2}h)
...
This shows that, with Simpson's rule, the finite element method
...
The extra term :math:`\frac{h^2}{6} u''` represents a smoothing: with just this
term, we would find u by integrating :math:`f` twice and thereby smooth :math:`f`
considerably. In addition, the finite element representation of :math:`f`
involves an average, or a smoothing, of :math:`f` on the right-hand side of
the equation system. If :math:`f` is a noisy function, direct interpolation
:math:`u_i=f_i` may result in a noisy :math:`u` too, but with a Galerkin or least
squares formulation and P1 elements, we should expect that :math:`u` is
smoother than :math:`f` unless :math:`h` is very small.

The interpretation that finite elements tend to smooth the solution
is valid in applications far beyond approximation of 1D function.

Making finite elements behave as finite differences
---------------------------------------------------

With a simple trick, using numerical integration, we can easily produce
the same result :math:`u_i=f_i` with the Galerkin or least square formulation
...
:math:`({\varphi}_i,{\varphi}_j)` involves a sum

.. math::
         \sum_k {\varphi}_i(x_{k}){\varphi}_j(x_{k}),

but :math:`{\varphi}_i(x_{k})=0` for all :math:`k`, except :math:`k=i`, and
:math:`{\varphi}_j(x_{k})=0` for all :math:`k`, except :math:`k=j`.
The product :math:`{\varphi}_i{\varphi}_j` is then different from zero only
when sampled at :math:`x_{i}` and :math:`i=j`. The approximation to the integral
is then
...
The matrix with entries :math:`({\varphi}_i,{\varphi}_j)` typically arises
from terms proportional to :math:`u` in a differential equation where :math:`u`
is the unknown function. This matrix is often called the *mass matrix*,
...
*mass lumping*. In time-dependent problems it can enhance the numerical
accuracy and computational efficiency of the finite element method.
However, there are also examples where mass lumping destroys accuracy.
...
in 2D and 3D. Then we define a set of *degrees of freedom*, which are
...
The concept of a *finite element* is now
  ...
  * a set of *degrees of freedom* that uniquely determine
  ...
  * a *mapping* of the reference cell onto to cell in the physical
    domain.
...
The expansion of :math:`u` over one cell is often used. In terms of reference
coordinates we have


.. math::
        
        u(x) = \sum_{r} c_r{\tilde{\varphi}}_r(X),
...
If we would approximate :math:`f` by piecewise constants, we simply
introduce one point or node in an element, preferably :math:`X=0`,
and choose :math:`{\tilde{\varphi}}_0(X)=1`. Only the ``dof_map`` is altered:
...
on the geometry of a cell, while ``dof_map`` is used in the
...
These four :math:`4\times 4` linear equations can be solved, yielding these formulas
...
All these rules apply equally spaced points. More accurate rules, for
a given :math:`M`, arise if the location of the points are optimized for
polynomial integrands.  The `Gauss-Legendre rules <http://en.wikipedia.org/wiki/Gaussian_quadrature>`_ (also known as
...
The code `numint.py <http://tinyurl.com/jvzzcfn/fem/numint.py>`_ contains a large collection of Gauss-Legendre rules.
...
:math:`{\varphi}_i(x,y)` or :math:`{\varphi}_i(x,y,z)` over some domain :math:`\Omega`, and
...
:math:`\hat{\varphi}_p(x)\hat{\varphi}_q(y)` for :math:`p\in{\mathcal{I}_x}=\{0,\ldots,N_x\}`
and :math:`q\in{\mathcal{I}_y}=\{0,\ldots,N_y\}`.
We can either work with double indices,
:math:`{\varphi}_{p,q}(x,y) = \hat{\varphi}_p(x)\hat{\varphi}_q(y)`, and write


.. math::
         u = \sum_{p\in{\mathcal{I}_x}}\sum_{q\in{\mathcal{I}_y}} c_{p,q}{\varphi}_{p,q}(x,y),
        

or we may transform the double index :math:`(p,q)` to a single index :math:`i`,
using :math:`i=p N_y + q` or :math:`i=q N_x + p`.

Suppose we choose :math:`\hat{\varphi}_p(x)=x^p`, and try an approximation with
:math:`N_x=N_y=1`:


.. math::
         {\varphi}_{0,0}=1,\quad {\varphi}_{1,0}=x, \quad {\varphi}_{0,1}=y,
        \quad {\varphi}_{1,1}=xy
        \thinspace .
...
.. math::
         {\varphi}_0=1,\quad {\varphi}_1=x, \quad {\varphi}_2=y,\quad{\varphi}_3 =xy
        \thinspace .
...
.. math::
        ...
        A_{0,0} &= ({\varphi}_0,{\varphi}_0) = \int_0^{L_y}\int_{0}^{L_x}
        {\varphi}_0(x,y)^2 dx dy = \int_0^{L_y}\int_{0}^{L_x}dx dy = L_xL_y,\\ 
        A_{1,0} &= ({\varphi}_1,{\varphi}_0) = \int_0^{L_y}\int_{0}^{L_x} x dxdy =
        \frac{1}{2}L_x^2L_y,\\ 
        A_{0,1} &= ({\varphi}_0,{\varphi}_1) = \int_0^{L_y}\int_{0}^{L_x} y dxdy =
        \frac{1}{2}L_y^2L_x,\\ 
        A_{0,1} &= ({\varphi}_0,{\varphi}_1) = \int_0^{L_y}\int_{0}^{L_x} xy dxdy =
...
.. math::
        ...
        A_{i,j} &= ({\varphi}_i,{\varphi}_j) = \int_0^{L_y}\int_{0}^{L_x}
        {\varphi}_i{\varphi}_j dx dy \\ 
        &= \int_0^{L_y}\int_{0}^{L_x}
        {\varphi}_{p,q}{\varphi}_{r,s} dx dy
        = \int_0^{L_y}\int_{0}^{L_x}
        \hat{\varphi}_p(x)\hat{\varphi}_q(y)\hat{\varphi}_r(x)\hat{\varphi}_s(y) dx dy\\ 
        &= \int_0^{L_y} \hat{\varphi}_q(y)\hat{\varphi}_s(y)dy
        \int_{0}^{L_x} \hat{\varphi}_p(x) \hat{\varphi}_r(x) dx\\ 
...
.. math::
         \hat A^{(x)}_{p,r} = \int_{0}^{L_x} \hat{\varphi}_p(x) \hat{\varphi}_r(x) dx,
        \quad
        \hat A^{(y)}_{q,s} = \int_0^{L_y} \hat{\varphi}_q(y)\hat{\varphi}_s(y)dy,
        

are matrix entries for one-dimensional approximations. Moreover,
:math:`i=q N_y+q` and :math:`j=s N_y+r`.

With :math:`\hat{\varphi}_p(x)=x^p` we have
...
.. math::
        ...
        b_i &= ({\varphi}_i,f) = \int_0^{L_y}\int_{0}^{L_x}{\varphi}_i f\,dxdx\\ 
        &= \int_0^{L_y}\int_{0}^{L_x}\hat{\varphi}_p(x)\hat{\varphi}_q(y) f\,dxdx\\ 
        &= \int_0^{L_y}\hat{\varphi}_q(y) (1+2y^2)dy
        \int_0^{L_y}\hat{\varphi}_p(x) x^p (1+x^2)dx\\ 
...
We may continue with adding higher powers to the basis and check that
with :math:`N_x\geq 2` and :math:`N_y\geq 2` we recover the exact function :math:`f`:
...
triangular element, :math:`u` is a linear function over each cell, with
discontinuous derivatives at the cell boundaries, as depicted in
Figure :ref:`fem:approx:fe:2D:fig:femfunc`.
...
be used for both P1 and higher-order elements.
...
we may use the basis functions of the actual element:
...
.. math::
        ...
        \int_{\Omega^{(e)}}{\varphi}_i (\boldsymbol{x}) {\varphi}_j (\boldsymbol{x}) d\boldsymbol{x} =
        \int_{\tilde\Omega^r} {\tilde{\varphi}}_i (\boldsymbol{X}) {\tilde{\varphi}}_j (\boldsymbol{X})
        \det J\, d\boldsymbol{X},
...
.. math::
        ...
        \int_{\Omega^{(e)}}{\varphi}_i (\boldsymbol{x}) f(\boldsymbol{x}) d\boldsymbol{x} =
        \int_{\tilde\Omega^r} {\tilde{\varphi}}_i (\boldsymbol{X}) f(\boldsymbol{x}(\boldsymbol{X})) \det J\, d\boldsymbol{X},
        

where :math:`d\boldsymbol{x} = dx dy` in 2D and :math:`d\boldsymbol{x} = dx dy dz` in 3D, with a similar
definition of :math:`d\boldsymbol{X}`. The quantity :math:`\det J` is the determinant of the
...
*ideas* and *concepts* as in 1D, but there is simply more to compute because the
...
*Hint.* One can make a loop over :math:`k` and call the ``least_squares`` and
...
:math:`{\varphi}_i(x) = sin((2i+1)x)`, :math:`i\in\mathcal{I}`.
Make a movie showing how :math:`u=\sum_{j\in\mathcal{I}}c_j{\varphi}_j(x)`
...
Given a function :math:`f(x)` on an interval :math:`[0,L]`, find the formula
for the coefficients of the Fourier series of :math:`f`:
...
.. _fem:approx:fe:exer:defmesh:

Exercise 8: Define finite element meshes
----------------------------------------

.. This exercise applies element/node terminology!
...
Exercise 12: Approximate a :math:`\tanh` function by P3 and P4 elements
-----------------------------------------------------------------------
...
A fundamental question is how accurate the finite element approximation
is in terms of the cell length :math:`h` and the degree :math:`d` of the basis
functions. We can investigate this empirically by choosing an :math:`f`
function, say :math:`f(x) = A\sin (\omega x)` on
:math:`\Omega=[0, 2\pi/\omega]`, and compute the approximation error
for a series of :math:`h` and :math:`d` values. The theory predicts that the
error should behave as :math:`h^{d+1}`. Use experiments to verify this
...
has the form :math:`c_i=f(x_{i})` for :math:`i\in\mathcal{I}`.
...
.. math::
         u(x) = \sum_{j\in\mathcal{I}} c_j{\psi}_j(x),

for some unknown coefficients :math:`c_i`, :math:`i\in\mathcal{I}`.
...
:math:`u(x) = B(x) + \sum_{j\in\mathcal{I}} c_j{\psi}_j(x)`,
...
:math:`N+1` unknowns :math:`c_i`, :math:`i\in\mathcal{I}`.
...
Various principles for determining :math:`c_i`, :math:`i\in\mathcal{I}`, try to minimize
:math:`R` in some sense. Note that :math:`R` varies with :math:`x` and
the :math:`c_i`, :math:`i\in\mathcal{I}`, parameters. We may write this dependence
explicitly as
...
The least-squares method aims to find :math:`c_i`, :math:`i\in\mathcal{I}`, so that
...
.. math::
        ...
        \min_{c_i, i\in\mathcal{I}} E = (R,R)\thinspace . 

Differentiating with respect to the free parameters :math:`c_i`, :math:`i\in\mathcal{I}`,
...
.. math::
   :label: fem:deq:1D:LS:eq1
        ...
        i=\mathcal{I} =\{0,\ldots,N\}\thinspace .
...
seeking :math:`c_i`, :math:`i\in\mathcal{I}`, such that
...
.. math::
   :label: fem:deq:1D:Galerkin
        ...
        (R,{\psi}_i)=0,\quad i\in\mathcal{I},
        
        

resulting in :math:`N+1` equations for determining :math:`c_i`, :math:`i\in\mathcal{I}`.
...
.. math::
   :label: fem:deq:1D:WRM
        ...
        (R,w_i)=0,\quad i\in\mathcal{I}\thinspace .
        
        

The result is :math:`N+1` equations for :math:`c_i`, :math:`i\in\mathcal{I}`.
...
.. math::
   :label: fem:deq:1D:collocation
        ...
        R(x_{i}; c_0,\ldots,c_N)=0,\quad i\in\mathcal{I}\thinspace .
...
.. math::
        ...
        \int_{\Omega_i} R\, {\, \mathrm{d}x}=0,\quad i\in\mathcal{I}\thinspace . 
...
.. math::
        ...
        \int_{\Omega} Rw_i\, {\, \mathrm{d}x}=0,\quad i\in\mathcal{I}, 
...
.. math::
   :label: fem:deq:1D:ex:sines:psi
        ...
        {\psi}_i(x) = {\sin\left((i+1)\pi\frac{x}{L}\right)},\quad i\in\mathcal{I}\thinspace .
...
.. math::
        ...
        = \frac{d^2}{dx^2}\left(\sum_{j\in\mathcal{I}} c_j{\psi}_j(x)\right)
        + f(x),\nonumber
...
.. math::
   :label: fem:deq:1D:ex:sines:res
        ...
        = -\sum_{j\in\mathcal{I}} c_j{\psi}_j''(x) + f(x)\thinspace .
...
.. math::
        ...
        \frac{\partial}{\partial c_i}
        \left(\sum_{j\in\mathcal{I}} c_j{\psi}_j''(x) + f(x)\right)
        = \sum_{j\in\mathcal{I}} \frac{\partial c_j}{\partial c_i}{\psi}_j''(x)
        = {\psi}_i''(x)\thinspace . 

The governing equations for :math:`c_i`, :math:`i\in\mathcal{I}`, are then


.. math::
        
        (\sum_j c_j {\psi}_j'' + f,{\psi}_i'')=0,\quad i\in\mathcal{I},
...
.. math::
        ...
        \sum_{j\in\mathcal{I}}({\psi}_i'',{\psi}_j'')c_j = -(f,{\psi}_i''),\quad i\in\mathcal{I}{\thinspace .}
...
basis functions :math:`v={\psi}_i`, :math:`i\in\mathcal{I}` (cf.
...
.. math::
        ...
        (\sum_{j\in\mathcal{I}} c_j{\psi}_j'', {\psi}_i)=-(f,{\psi}_i),\quad i\in\mathcal{I}\thinspace . 

This equation can be rearranged to a form that explicitly shows
that we get a linear system for the unknowns :math:`c_i`, :math:`i\in\mathcal{I}`:
...
.. math::
        ...
        -\sum_{j\in\mathcal{I}} c_j{\psi}_j''(x_{i}) = f(x_{i}),\quad i\in\mathcal{I},
...
for :math:`i\in\mathcal{I}`. What about a boundary condition like :math:`u(L)=D\neq0`?
...
.. math::
         u(x) = B(x) + \sum_{j\in\mathcal{I}} c_j{\psi}_j(x)
        \thinspace .
...
.. math::
         u(L) = B(L) + \sum_{j\in\mathcal{I}} c_j{\psi}_j(L) = B(L) = D\thinspace . 
...
.. math::
   :label: fem:deq:1D:essBC:Bfunc:u1
        ...
        u(x) = \frac{x}{L}D + \sum_{j\in\mathcal{I}} c_j{\psi}_j(x),
...
.. math::
         \sum_{j\in\mathcal{I}} a({\psi}_j,{\psi}_i) c_j = L({\psi}_i),\quad i\in\mathcal{I}
        \thinspace .
...
.. math::
         u(x) = B(x) + \sum_{j\in\mathcal{I}} c_j{\psi}_i(x),

with :math:`{\psi}_i(0)={\psi}_i(L)=0` for :math:`i\in\mathcal{I}`. The function :math:`B(x)`
...
.. math::
        ...
        (aB' + a\sum_{j\in\mathcal{I}} c_j {\psi}_j', {\psi}_i') =
        (f,{\psi}_i), \quad i\in\mathcal{I} \thinspace .
...
.. math::
         \sum_{j\in\mathcal{I}} (a{\psi}_j', {\psi}_i')c_j  =
        (f,{\psi}_i) + (a(D-C)L^{-1}, {\psi}_i'), \quad i\in\mathcal{I}
        \thinspace .
...
.. math::
         u = \frac{C}{L}(L-x) + \sum_{j\in\mathcal{I}} c_j {\psi}_i(x)
        \thinspace .
...
and replacing :math:`v` by :math:`{\psi}_i` for :math:`i\in\mathcal{I}`. Some algebra results in


.. math::
         \sum_{j\in\mathcal{I}} \underbrace{(({\psi}_j',{\psi}_i') + (b{\psi}_j',{\psi}_i))}_{A_{i,j}} c_j = \underbrace{(f,{\psi}_i) + (bCL^{-1},{\psi}_i') + E {\psi}_i(L)}_{b_i}
        \thinspace .
...
algebraic equations* for the unknowns :math:`c_i`, :math:`i\in\mathcal{I}`. Such systems must
...
:math:`{\psi}_i(x)=(1-x)^{i+1}`, :math:`i\in\mathcal{I}`. A suitable :math:`B(x)` function
...
.. math::
         \bar F(c_0,\ldots,c_N) = \sum_{j\in\mathcal{I}}\sum_{i\in\mathcal{I}} a({\psi}_i,{\psi}_j)c_ic_j - \sum_{j\in\mathcal{I}} L({\psi}_j)c_j
...
:math:`{\psi}_i(0)={\psi}_i(L)=0`, :math:`i\in\mathcal{I}`. When :math:`{\psi}_i`,
...
.. math::
        ...
        b_i=\int_0^L2{\psi}_i(x) {\, \mathrm{d}x}, \quad i,j\in\mathcal{I}{\thinspace .}
...
.. math::
        ...
        b_i=\int_0^L2{\varphi}_{i+1}(x) {\, \mathrm{d}x},\quad i,j\in\mathcal{I}{\thinspace .}
...
:math:`c_i` for :math:`i\in\mathcal{I}`. With our numbering of unknowns and
...
.. math::
        ...
        \sum_{j\in\mathcal{I}} c_j{\varphi}_{j+1}(x)\thinspace .
...
.. math::
         {\psi}_i = {\varphi}_{i+1},\quad i\in\mathcal{I} = \{0,\ldots,N=n_n-1\}{\thinspace .}
...
.. math::
        ...
        \sum_{j\in\mathcal{I}} c_j{\varphi}_{j+1}(x)
...
:eq:`fem:deq:1D:ex1:Ab:glob`. If we include all the :math:`c_i`, :math:`i\in\mathcal{I}`,
...
for :math:`i\in\mathcal{I}`, times the boundary value :math:`K`, from the
...
and finally set :math:`b_k=K`. The steps in algorithmic form becomes
 ...
 1. :math:`b_i \leftarrow b_i - A_{i,k}K` for :math:`i\in\mathcal{I}`

 2. :math:`A_{i,k} = A_{k,i} = 0` for :math:`i\in\mathcal{I}`
...
.. math::
         \int_0^L(u''(x)+f(x)){\psi}_i(x) {\, \mathrm{d}x} = 0,\quad i\in\mathcal{I},
...
.. math::
        ...
        \int_0^L f(x){\varphi}_i(x) {\, \mathrm{d}x},\quad i\in\mathcal{I}\thinspace . 
...
.. math::
   :label: fem:deq:1D:natBC2
        ...
        \int_0^L\left(f(x){\varphi}_i(x)\right) {\, \mathrm{d}x}
         - C{\varphi}_i(0),\quad i\in\mathcal{I}
        \thinspace .
...
.. math::
         V = \hbox{span}\,\{{\psi}_i(x) = x^{i+1}(1-x)\}_{i\in\mathcal{I}}\thinspace .
...
.. math::
         u(x) = B(x) + \bar u (x),\quad \bar u(x) = \sum_{j\in\mathcal{I}} c_j{\psi}_j(x)\thinspace . 
...
.. math::
        ...
        \sum_{j\in\mathcal{I}}\left(\int_0^1{\psi}_i'{\psi}_j' {\, \mathrm{d}x}\right)c_j = \int_0^1(f-B'){\psi}_i {\, \mathrm{d}x},
        \quad i\in\mathcal{I}\thinspace . 
...
.. math::
         u = u_0 + \sum_{j\in\mathcal{I}} c_j{\psi}_j \thinspace .
...
However, we can also keep all the :math:`c_j`, :math:`j\in\mathcal{I}`, as unknowns
...
:eq:`fem:deq:1D:ex1:Ab:glob3`, where all :math:`c_i`, :math:`i\in\mathcal{I}`, are
