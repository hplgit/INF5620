...
.. math::
         u = \sum_{j\in{I}} c_j{\psi}_j(x,y){\thinspace .}
...
:math:`{\psi}_i(x) = \sin((2i+1)x)`, :math:`i\in{I} = \{0,\ldots,N\}`.
Make a movie showing how :math:`u=\sum_{j\in{I}}c_j{\psi}_j(x)`
...
of the basis functions :math:`\left\{ {{\varphi}}_i \right\}_{i\in{I}}`, with :math:`{I}` as
the index set :math:`\{0,\ldots,N\}`:


.. math::
         u(x) = \sum_{j\in{I}} c_j{\psi}_j(x){\thinspace .}

The coefficients :math:`\left\{ {c}_i \right\}_{i\in{I}}` are unknowns to be computed.
...
:math:`N+1` unknowns :math:`\left\{ {c}_i \right\}_{i\in{I}}`.
...
Various principles for determining :math:`\left\{ {c}_i \right\}_{i\in{I}}` try to minimize
:math:`R` in some sense. Note that :math:`R` varies with :math:`x` and
the :math:`\left\{ {c}_i \right\}_{i\in{I}}` parameters. We may write this dependence
explicitly as
...
The least-squares method aims to find :math:`\left\{ {c}_i \right\}_{i\in{I}}` such that
...
Differentiating with respect to the free parameters :math:`\left\{ {c}_i \right\}_{i\in{I}}`
...
seeking :math:`\left\{ {c}_i \right\}_{i\in{I}}` such that
...
.. math::
   :label: fem:deq:1D:Galerkin
        ...
        (R,{\psi}_i)=0,\quad i\in{I},
        
        

resulting in :math:`N+1` equations for determining :math:`\left\{ {c}_i \right\}_{i\in{I}}`.
...
.. math::
   :label: fem:deq:1D:WRM
        ...
        (R,w_i)=0,\quad i\in{I}{\thinspace .}
        
        

The result is :math:`N+1` equations for :math:`\left\{ {c}_i \right\}_{i\in{I}}`.
...
.. math::
   :label: fem:deq:1D:collocation
        ...
        R(x_{i}; c_0,\ldots,c_N)=0,\quad i\in{I}{\thinspace .}
...
.. math::
        ...
        \int_{\Omega_i} R\, {\, \mathrm{d}x}=0,\quad i\in{I}{\thinspace .}  
...
.. math::
        ...
        \int_{\Omega} Rw_i\, {\, \mathrm{d}x}=0,\quad i\in{I}, 
...
.. math::
   :label: fem:deq:1D:ex:sines:psi
        ...
        {\psi}_i(x) = {\sin\left((i+1)\pi\frac{x}{L}\right)},\quad i\in{I}{\thinspace .}
...
.. math::
   :label: fem:deq:1D:ex:sines:res
        ...
        = \sum_{j\in{I}} c_j{\psi}_j''(x) + f(x){\thinspace .}
...
The governing equations for :math:`\left\{ {c}_i \right\}_{i\in{I}}` are then


.. math::
        
        (\sum_j c_j {\psi}_j'' + f,{\psi}_i'')=0,\quad i\in{I},
...
.. math::
        ...
        \sum_{j\in{I}}({\psi}_i'',{\psi}_j'')c_j = -(f,{\psi}_i''),\quad i\in{I}{\thinspace .}
...
basis functions :math:`v={\psi}_i`, :math:`i\in{I}`, see
...
.. math::
        ...
        (\sum_{j\in{I}} c_j{\psi}_j'', {\psi}_i)=-(f,{\psi}_i),\quad i\in{I}{\thinspace .}  

This equation can be rearranged to a form that explicitly shows
that we get a linear system for the unknowns :math:`\left\{ {c}_i \right\}_{i\in{I}}`:
...
.. math::
        ...
        -\sum_{j\in{I}} c_j{\psi}_j''(x_{i}) = f(x_{i}),\quad i\in{I},
...
for :math:`i\in{I}`. What about a boundary condition like :math:`u(L)=D\neq0`?
...
.. math::
         u(L) = B(L) + \sum_{j\in{I}} c_j{\psi}_j(L) = B(L) = D{\thinspace .}  
...
.. math::
   :label: fem:deq:1D:essBC:Bfunc:u1
        ...
        u(x) = \frac{x}{L}D + \sum_{j\in{I}} c_j{\psi}_j(x),
...
.. math::
         \sum_{j\in{I}} a({\psi}_j,{\psi}_i) c_j = L({\psi}_i),\quad i\in{I},
...
.. math::
         \bar F(c_0,\ldots,c_N) = \sum_{j\in{I}}\sum_{i\in{I}} a({\psi}_i,{\psi}_j)c_ic_j - \sum_{j\in{I}} L({\psi}_j)c_j
...
.. math::
         \frac{\partial\bar F}{\partial c_i}=0,\quad i\in{I}{\thinspace .}

After some algebra one finds


.. math::
         \sum{j\in{I}} a({\psi}_i,{\psi}_j)c_j = L({\psi}_i),\quad i\in{I},
...
.. math::
         u(x) = B(x) + \sum_{j\in{I}} c_j{\psi}_i(x),

with :math:`{\psi}_i(0)={\psi}_i(L)=0` for :math:`i\in{I}`. The function :math:`B(x)`
...
.. math::
         {\psi}_i(0)={\psi}_i(L)=0, \quad i\in{I}{\thinspace .}
...
we force :math:`{\psi}_i(0)=0` and use a boundary function
...
and replacing :math:`v` by :math:`{\psi}_i` for :math:`i\in{I}`. Some algebra results in
...
Our space :math:`V` has basis :math:`\left\{ {{\psi}}_i \right\}_{i\in{I}}`, and because of the
condition :math:`u(0)=0`, we must require :math:`{\psi}_i(0)=0`, :math:`i\in{I}`.
...
algebraic equations* for the unknowns :math:`c_i`, :math:`i\in{I}`. Such systems must
...
.. math::
         {\psi}_i(x)=(1-x)^{i+1}, \quad i\in{I}{\thinspace .}
...
:math:`{\psi}_i(0)={\psi}_i(L)=0`, :math:`i\in{I}`. When :math:`{\psi}_i`,
...
.. math::
        ...
        b_i=\int_0^L2{\psi}_i(x) {\, \mathrm{d}x}, \quad i,j\in{I}{\thinspace .}
...
.. math::
        ...
        b_i=\int_0^L2{\varphi}_{i+1}(x) {\, \mathrm{d}x},\quad i,j\in{I}{\thinspace .}
...
:math:`c_i` for :math:`i\in{I}`. With our numbering of unknowns and
...
.. math::
        ...
        u(x) &= C{\varphi}_0 + D{\varphi}_{N_n} + \sum_{j\in{I}} c_j{\varphi}_{j+1}
...
We choose :math:`{\psi}_i={\varphi}_{i+1}` if the node numbering is from left
...
.. math::
         B(x) = 0\cdot{\varphi}_0(x) + D{\varphi}_{N_n}(x) = D{\varphi}_{N_n}(x){\thinspace .}

The expansion for :math:`u(x)` is


.. math::
         u(x)  = B(x) + \sum_{j\in{I}} c_j{\varphi}_{j+1}(x){\thinspace .} 

The limit :math:`N` of the index set :math:`{I}` equals :math:`N_n-2`.
...
:math:`-D/2` to :math:`b_{N}=b_{N_n-1}`.
...
We observe that :math:`B(x)=D{\varphi}_{N_n}'=0` on all cells except the last
one. Here there is only one unknown since :math:`u(L)` is prescribed, so
...
for :math:`e=N_e=N_n-1`.
When assembling these contributions, we see that :math:`b_{N}` in the
...
.. math::
   :label: fem:deq:1D:fem:essBC:Bfunc:modsys:uall
        ...
        u(x) = \sum_{j\in{I}}c_j{\varphi}_j(x){\thinspace .}
        
        

Note that the collections of unknowns
:math:`\left\{ {c}_i \right\}_{i\in{I}}` in :eq:`fem:deq:1D:fem:essBC:Bfunc:modsys:utrad`
...
for :math:`i\in{I}`, times the boundary value :math:`U_k`, from the
...
and finally set :math:`b_k=U_k`. The steps in algorithmic form becomes
 ...
 1. :math:`b_i \leftarrow b_i - A_{i,k}U_k` for :math:`i\in{I}`

 2. :math:`A_{i,k} = A_{k,i} = 0` for :math:`i\in{I}`
...
.. math::
        ...
        \int_0^L(u''(x)+f(x)){\psi}_i(x) {\, \mathrm{d}x} = 0,\quad i\in{I},
...
.. math::
        ...
        \int_0^L f(x){\psi}_i(x) {\, \mathrm{d}x}, \quad i\in{I}{\thinspace .}
...
.. math::
   :label: fem:deq:1D:BC:nat:varform
        ...
        \int_0^L f(x){\varphi}_i(x) {\, \mathrm{d}x},\quad i\in{I}{\thinspace .}
...
Here, :math:`u(L)=D` and then :math:`{\psi}_i(0)=0`, :math:`i\in{I}`. Obviously,
the boundary term :math:`u'(L){\psi}_i(L)` then vanishes.

The set of basis functions :math:`\left\{ {{\psi}}_i \right\}_{i\in{I}}` contains in this
...
a basis :math:`\left\{ {{\psi}}_i \right\}_{i\in{I}}` which contains all the finite element
...
is :math:`{\psi}_i(x) = x^{i+1}(1-x)`, :math:`i\in{I}`. Note that
...
.. math::
         u(x) = B(x) + \bar u (x),\quad \bar u(x) = \sum_{j\in{I}} c_j{\psi}_j(x){\thinspace .}  
...
.. math::
         u = u_0 + \sum_{j\in{I}} c_j{\psi}_j {\thinspace .} 
...
However, we can also keep all the :math:`c_j`, :math:`j\in{I}`, as unknowns
...
.. math::
        ...
        u_1 = \sum_{j=0}^{N_s} c_{1,j}{\psi}(\boldsymbol{x})
...
.. math::
        ...
        f &= ((f(\boldsymbol{x},t_n),{\psi}_i), \ldots, (f(\boldsymbol{x},t_n),{\psi}_{N_s})),\\ 
        c &=(c_0,\ldots,c_{N_s}),\\ 
...
2. Initialize :math:`u^0` by
  ...
  2. projection: solve
     :math:`\sum_j M_{i,j}c_{1,j} = (I,{\psi}_i)`, :math:`i=0,\ldots,N_s`.
...
:eq:`fem:deq:1D:ex1:Ab:glob3`, where all :math:`\left\{ {c}_i \right\}_{i\in{I}}` are
