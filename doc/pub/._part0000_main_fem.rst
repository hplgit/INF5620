...
.. math::
         (\boldsymbol{f} - \sum_{j=0}^N c_j\boldsymbol{\psi}_j, \boldsymbol{\psi}_i) = (\boldsymbol{f}, \boldsymbol{\psi}_i) - \sum_{j\in{\mathcal{I}_V}}
...
.. math::
   :label: fem:approx:ufem
        ...
        u = \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j{\thinspace .}
        
        

The index set :math:`{\mathcal{I}_V}` is defined as :math:`{\mathcal{I}_V} =\{0,\ldots,N\}` and is used
...
:math:`u=u(x)`, :math:`{\psi}_i={\psi}_i(x)`, :math:`i\in{\mathcal{I}_V}`. Later, we will almost
...
.. math::
   :label: fem:approx:LS:E
        ...
        E = (f(x)-\sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j(x), f(x)-\sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j(x)){\thinspace .}
...
.. math::
        ...
        E(c_i, \ldots, c_N) = (f,f) -2\sum_{j\in{\mathcal{I}_V}} c_j(f,{\psi}_i)
        + \sum_{p\in{\mathcal{I}_V}}\sum_{q\in{\mathcal{I}_V}} c_pc_q({\psi}_p,{\psi}_q){\thinspace .}  

Minimizing this function of :math:`N+1` scalar variables
:math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}`, requires differentiation
with respect to :math:`c_i`, for all :math:`i\in{\mathcal{I}_V}`. The resulting
...
.. math::
   :label: fem:approx:Galerkin0
        ...
        (e,{\psi}_i)=0,\quad i\in{\mathcal{I}_V}{\thinspace .}
...
the symbol ``x``, ``psi`` is a list of expressions for :math:`\left\{ {{\psi}}_i \right\}_{i\in{\mathcal{I}_V}}`,
...
terms of the basis functions, :math:`f=\sum_{j\in{\mathcal{I}_V}} d_j{\psi}_j`, for
some coefficients :math:`\left\{ {d}_i \right\}_{i\in{\mathcal{I}_V}}`,
and the right-hand side then has entries


.. math::
         b_i = (f,{\psi}_i) = \sum_{j\in{\mathcal{I}_V}} d_j({\psi}_j, {\psi}_i) = \sum_{j\in{\mathcal{I}_V}} d_jA_{i,j}
        {\thinspace .}  

The linear system :math:`\sum_jA_{i,j}c_j = b_i`, :math:`i\in{\mathcal{I}_V}`, is then


.. math::
         \sum_{j\in{\mathcal{I}_V}} c_jA_{i,j} = \sum_{j\in{\mathcal{I}_V}}d_jA_{i,j},
        \quad i\in{\mathcal{I}_V},

which implies that :math:`c_i=d_i` for :math:`i\in{\mathcal{I}_V}`.
...
.. math::
         {\psi}_i(x) = \sin ((i+1)\pi x),\quad i\in{\mathcal{I}_V}{\thinspace .} 
...
.. math::
        ...
        u(x) = f(0) + \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j(x)
        {\thinspace .}
...
.. math::
        ...
        u(x) = f(0)(1-x) + xf(1) + \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j(x)
        {\thinspace .}
...
.. math::
        ...
        c_i = 2\int_0^1 f(x)\sin ((i+1)\pi x) {\, \mathrm{d}x},\quad i\in{\mathcal{I}_V},
...
:math:`x_{i}`, :math:`i\in{\mathcal{I}_V}`:


.. math::
        
        u(x_{i}) = \sum_{j\in{\mathcal{I}_V}} c_j {\psi}_j(x_{i}) = f(x_{i}),
        \quad i\in{\mathcal{I}_V}{\thinspace .} 

This criterion also gives a linear system
with :math:`N+1` unknown coefficients :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}`:


.. math::
        
        \sum_{j\in{\mathcal{I}_V}} A_{i,j}c_j = b_i,\quad i\in{\mathcal{I}_V},
...
Given :math:`f`  as a ``sympy`` symbolic expression ``f``, :math:`\left\{ {{\psi}}_i \right\}_{i\in{\mathcal{I}_V}}`
as a list ``psi``, and a set of points :math:`\left\{ {x}_i \right\}_{i\in{\mathcal{I}_V}}`  as a list or array
``points``, the following Python function sets up and solves the matrix system
for the coefficients :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}`:
...
a diagonal matrix: formulas for the coefficients :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}` can
...
.. math::
        ...
        c_i = f(x_{i}),\quad i\in{\mathcal{I}_V},
...
.. math::
        ...
        u(x) = \sum_{j\in{\mathcal{I}_V}} f(x_{i}){\psi}_i(x){\thinspace .}  
...
.. math::
        ...
        u(x_{i}) = \sum_{j\in{\mathcal{I}_V}} c_j {\psi}_j (x_{i}) =
        \sum_{j\in{\mathcal{I}_V}} c_j {\varphi}_j (x_{i}) = c_i {\varphi}_i (x_{i}) = c_i
        {\thinspace .}
...
we can compute the coefficients :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}` in the expansion
...
.. math::
         u(x_{i}) = \sum_{j\in{\mathcal{I}_V}} c_j{\varphi}_j(x_{i}) = f(x_{i}),\quad
        i\in{\mathcal{I}_V}{\thinspace .}
...
.. math::
        ...
        \sum_{j\in{\mathcal{I}_V}} c_j ({\psi}_i,{\psi}_j) = (f,{\psi}_i),\quad i\in{\mathcal{I}_V}{\thinspace .}
...
.. math::
         c_i = f_i,\quad i\in{\mathcal{I}_V}{\thinspace .}  
...
.. math::
         u = \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j(x,y){\thinspace .}
...
:math:`{\psi}_i(x) = \sin((2i+1)x)`, :math:`i\in{\mathcal{I}_V} = \{0,\ldots,N\}`.
Make a movie showing how :math:`u=\sum_{j\in{\mathcal{I}_V}}c_j{\psi}_j(x)`
...
has the form :math:`c_i=f(x_{i})` for :math:`i\in{\mathcal{I}_V}`.
...
of the basis functions :math:`\left\{ {{\varphi}}_i \right\}_{i\in{\mathcal{I}_V}}`, with :math:`{\mathcal{I}_V}` as
the index set :math:`\{0,\ldots,N\}`:


.. math::
         u(x) = \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j(x){\thinspace .}

The coefficients :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}` are unknowns to be computed.
...
:math:`N+1` unknowns :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}`.
...
Various principles for determining :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}` try to minimize
:math:`R` in some sense. Note that :math:`R` varies with :math:`x` and
the :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}` parameters. We may write this dependence
explicitly as
...
The least-squares method aims to find :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}` such that
...
Differentiating with respect to the free parameters :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}`
...
seeking :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}` such that
...
.. math::
   :label: fem:deq:1D:Galerkin
        ...
        (R,{\psi}_i)=0,\quad i\in{\mathcal{I}_V},
        
        

resulting in :math:`N+1` equations for determining :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}`.
...
.. math::
   :label: fem:deq:1D:WRM
        ...
        (R,w_i)=0,\quad i\in{\mathcal{I}_V}{\thinspace .}
        
        

The result is :math:`N+1` equations for :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}`.
...
.. math::
   :label: fem:deq:1D:collocation
        ...
        R(x_{i}; c_0,\ldots,c_N)=0,\quad i\in{\mathcal{I}_V}{\thinspace .}
...
.. math::
        ...
        \int_{\Omega_i} R\, {\, \mathrm{d}x}=0,\quad i\in{\mathcal{I}_V}{\thinspace .}  
...
.. math::
        ...
        \int_{\Omega} Rw_i\, {\, \mathrm{d}x}=0,\quad i\in{\mathcal{I}_V}, 
...
.. math::
   :label: fem:deq:1D:ex:sines:psi
        ...
        {\psi}_i(x) = {\sin\left((i+1)\pi\frac{x}{L}\right)},\quad i\in{\mathcal{I}_V}{\thinspace .}
...
.. math::
        ...
        = \frac{d^2}{dx^2}\left(\sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j(x)\right)
        + f(x),\nonumber
...
.. math::
   :label: fem:deq:1D:ex:sines:res
        ...
        = \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j''(x) + f(x){\thinspace .}
...
.. math::
        ...
        \frac{\partial}{\partial c_i}
        \left(\sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j''(x) + f(x)\right)
        = \sum_{j\in{\mathcal{I}_V}} \frac{\partial c_j}{\partial c_i}{\psi}_j''(x)
        = {\psi}_i''(x){\thinspace .}  

The governing equations for :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}` are then


.. math::
        
        (\sum_j c_j {\psi}_j'' + f,{\psi}_i'')=0,\quad i\in{\mathcal{I}_V},
...
.. math::
        ...
        \sum_{j\in{\mathcal{I}_V}}({\psi}_i'',{\psi}_j'')c_j = -(f,{\psi}_i''),\quad i\in{\mathcal{I}_V}{\thinspace .}
...
basis functions :math:`v={\psi}_i`, :math:`i\in{\mathcal{I}_V}`, see
...
.. math::
        ...
        (\sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j'', {\psi}_i)=-(f,{\psi}_i),\quad i\in{\mathcal{I}_V}{\thinspace .}  

This equation can be rearranged to a form that explicitly shows
that we get a linear system for the unknowns :math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}`:
...
.. math::
        ...
        -\sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j''(x_{i}) = f(x_{i}),\quad i\in{\mathcal{I}_V},
...
for :math:`i\in{\mathcal{I}_V}`. What about a boundary condition like :math:`u(L)=D\neq0`?
...
.. math::
         u(x) = B(x) + \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j(x)
        {\thinspace .}
...
.. math::
         u(L) = B(L) + \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j(L) = B(L) = D{\thinspace .}  
...
.. math::
   :label: fem:deq:1D:essBC:Bfunc:u1
        ...
        u(x) = \frac{x}{L}D + \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j(x),
...
.. math::
         \sum_{j\in{\mathcal{I}_V}} a({\psi}_j,{\psi}_i) c_j = L({\psi}_i),\quad i\in{\mathcal{I}_V},
...
.. math::
         \bar F(c_0,\ldots,c_N) = \sum_{j\in{\mathcal{I}_V}}\sum_{i\in{\mathcal{I}_V}} a({\psi}_i,{\psi}_j)c_ic_j - \sum_{j\in{\mathcal{I}_V}} L({\psi}_j)c_j
...
.. math::
         \frac{\partial\bar F}{\partial c_i}=0,\quad i\in{\mathcal{I}_V}{\thinspace .}

After some algebra one finds


.. math::
         \sum{j\in{\mathcal{I}_V}} a({\psi}_i,{\psi}_j)c_j = L({\psi}_i),\quad i\in{\mathcal{I}_V},
...
.. math::
         u(x) = B(x) + \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_i(x),

with :math:`{\psi}_i(0)={\psi}_i(L)=0` for :math:`i\in{\mathcal{I}_V}`. The function :math:`B(x)`
...
.. math::
         {\psi}_i(0)={\psi}_i(L)=0, \quad i\in{\mathcal{I}_V}{\thinspace .}
...
.. math::
        ...
        ({\alpha} B' + {\alpha} \sum_{j\in{\mathcal{I}_V}} c_j {\psi}_j', {\psi}_i') =
        (f,{\psi}_i), \quad i\in{\mathcal{I}_V} {\thinspace .}
...
.. math::
         \sum_{j\in{\mathcal{I}_V}} ({\alpha}{\psi}_j', {\psi}_i')c_j  =
        (f,{\psi}_i) + (a(D-C)L^{-1}, {\psi}_i'), \quad i\in{\mathcal{I}_V}
        {\thinspace .}
...
.. math::
         u = C + \sum_{j\in{\mathcal{I}_V}} c_j {\psi}_i(x)
        {\thinspace .}
...
and replacing :math:`v` by :math:`{\psi}_i` for :math:`i\in{\mathcal{I}_V}`. Some algebra results in


.. math::
         \sum_{j\in{\mathcal{I}_V}} \underbrace{(({\psi}_j',{\psi}_i') + (b{\psi}_j',{\psi}_i))}_{A_{i,j}} c_j = \underbrace{(f,{\psi}_i) + E {\psi}_i(L)}_{b_i}
        {\thinspace .}
...
Our space :math:`V` has basis :math:`\left\{ {{\psi}}_i \right\}_{i\in{\mathcal{I}_V}}`, and because of the
condition :math:`u(0)=0`, we must require :math:`{\psi}_i(0)=0`, :math:`i\in{\mathcal{I}_V}`.
...
algebraic equations* for the unknowns :math:`c_i`, :math:`i\in{\mathcal{I}_V}`. Such systems must
...
.. math::
         {\psi}_i(x)=(1-x)^{i+1}, \quad i\in{\mathcal{I}_V}{\thinspace .}
...
:math:`{\psi}_i(0)={\psi}_i(L)=0`, :math:`i\in{\mathcal{I}_V}`. When :math:`{\psi}_i`,
...
.. math::
        ...
        b_i=\int_0^L2{\psi}_i(x) {\, \mathrm{d}x}, \quad i,j\in{\mathcal{I}_V}{\thinspace .}
...
.. math::
        ...
        b_i=\int_0^L2{\varphi}_{i+1}(x) {\, \mathrm{d}x},\quad i,j\in{\mathcal{I}_V}{\thinspace .}
...
:math:`c_i` for :math:`i\in{\mathcal{I}_V}`. With our numbering of unknowns and
...
.. math::
        ...
        u(x) = \sum_{j\in{I_b}} U_j{\varphi}_j(x) + \sum{j\in{\mathcal{I}_V}}c_j{\varphi}_{\nu(j)},
...
.. math::
        ...
        hbox{ only for }j=k} +
        \sum_{j\in{\mathcal{I}_V}} c_j\underbrace{{\varphi}_{\nu(j)}(x_{k})}_{=0,\ k\not\in{\mathcal{I}_V}}
        = U_k 
...
and Dirichlet conditions at :math:`x=0`, we
have :math:`\nu(j)=j+1`, :math:`j=0,\ldots,N=N_n-1`.
...
.. math::
        ...
        u(x) &= C{\varphi}_0 + D{\varphi}_{N_n} + \sum_{j\in{\mathcal{I}_V}} c_j{\varphi}_{j+1}
...
.. math::
         u(x)  = B(x) + \sum_{j\in{\mathcal{I}_V}} c_j{\varphi}_{j+1}(x){\thinspace .} 
...
The element matrices and vectors are calculated as the section :ref:`fem:deq:1D:comp:elmwise`, so now we concentrate on the impact of
...
Here there is only one unknown in these cells
...
.. math::
   :label: fem:deq:1D:fem:essBC:Bfunc:modsys:utrad
        ...
        + \sum_{j\in{\mathcal{I}_V}}c_j{\varphi}_{\nu(j)}(x),
...
.. math::
   :label: fem:deq:1D:fem:essBC:Bfunc:modsys:uall
        ...
        u(x) = \sum_{j\in{\mathcal{I}_V}}c_j{\varphi}_j(x){\thinspace .}
        
        

Note that the collections of unknowns
:math:`\left\{ {c}_i \right\}_{i\in{\mathcal{I}_V}}` in :eq:`fem:deq:1D:fem:essBC:Bfunc:modsys:utrad`
...
in :eq:`fem:deq:1D:fem:essBC:Bfunc:modsys:utrad` :math:`N=N_n`.
The number of unknowns in :eq:`fem:deq:1D:fem:essBC:Bfunc:modsys:utrad`
...
.. math::
        ...
        \int_0^L 2{\varphi}_0(x) {\, \mathrm{d}x} = h,\\ 
...
Let :math:`c_k` be a coefficient corresponding to a known value :math:`U_k`.
...
for :math:`i\in{\mathcal{I}_V}`, times the boundary value :math:`U_k`, from the
...
and finally set :math:`b_k=U_k`. The steps in algorithmic form becomes
 ...
 1. :math:`b_i \leftarrow b_i - A_{i,k}U_k` for :math:`i\in{\mathcal{I}_V}`

 2. :math:`A_{i,k} = A_{k,i} = 0` for :math:`i\in{\mathcal{I}_V}`
...
.. math::
        ...
        \int_0^L(u''(x)+f(x)){\psi}_i(x) {\, \mathrm{d}x} = 0,\quad i\in{\mathcal{I}_V},
...
.. math::
        ...
        \int_0^L f(x){\psi}_i(x) {\, \mathrm{d}x}, \quad i\in{\mathcal{I}_V}{\thinspace .}
...
.. math::
   :label: fem:deq:1D:BC:nat:varform
        ...
        \int_0^L f(x){\varphi}_i(x) {\, \mathrm{d}x},\quad i\in{\mathcal{I}_V}{\thinspace .}
...
Here, :math:`u(L)=D` and then :math:`{\psi}_i(0)=0`, :math:`i\in{\mathcal{I}_V}`. Obviously,
the boundary term :math:`u'(L){\psi}_i(L)` then vanishes.

The set of basis functions :math:`\left\{ {{\psi}}_i \right\}_{i\in{\mathcal{I}_V}}` contains in this
...
a basis :math:`\left\{ {{\psi}}_i \right\}_{i\in{\mathcal{I}_V}}` which contains all the finite element
...
.. math::
        ...
        u(x) = \sum_{j\in{\mathcal{I}_V}} c_j{\varphi}_j(x),
...
.. math::
   :label: fem:deq:1D:natBC2
        ...
        \sum_{j\in{\mathcal{I}_V}}\left(
        \int_0^L {\varphi}_i'(x){\varphi}_j'(x) {\, \mathrm{d}x} \right)c_j =
        \int_0^L\left(f(x){\varphi}_i(x)\right) {\, \mathrm{d}x}
         - C{\varphi}_i(0),\quad i\in{\mathcal{I}_V}
        {\thinspace .}
...
is :math:`{\psi}_i(x) = x^{i+1}(1-x)`, :math:`i\in{\mathcal{I}_V}`. Note that
...
.. math::
        ...
        u(x) = B(x) + \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j(x){\thinspace .}
...
.. math::
        ...
        \sum_{j\in{\mathcal{I}_V}}\left(\int_0^1{\psi}_i'{\psi}_j' {\, \mathrm{d}x}\right)c_j = \int_0^1(f-B'){\psi}_i {\, \mathrm{d}x},
        \quad i\in{\mathcal{I}_V}{\thinspace .}
...
.. math::
         u = B + \sum_{j\in{\mathcal{I}_V}} c_j{\psi}_j,\quad B = u_0 {\thinspace .} 
...
.. math::
        ...
        \sum_{j\in{\mathcal{I}_V}} ((\boldsymbol{v}\cdot\nabla {\psi}_j, {\psi}_i) &+ (\alpha {\psi}_j ,{\psi}_i) + (a\nabla {\psi}_j,\nabla {\psi}_i))c_j = \\ 
...
However, we can also keep all the :math:`c_j`, :math:`j\in{\mathcal{I}_V}`, as unknowns
...
a triangle have 1 and 3 points:
        ...
        >>> import numint
        >>> x, w = numint.quadrature_for_triangles(1)
        >>> x
        [(0.3333333333333333, 0.3333333333333333)]
        >>> w
        [0.5]
        >>> x, w = numint.quadrature_for_triangles(3)
        >>> x
...
in the spatial (s) domain. The number of time points is denoted by :math:`N_t`.
...
.. math::
         \int_\Omega Rw_i{\, \mathrm{d}x} = 0,\quad i=0,\ldots,N,
...
The Galerkin method corresponds to choosing :math:`w_i={\psi}_i`.
...
.. math::
        ...
        \int_{\partial\Omega}{\alpha}\frac{\partial u^n}{\partial n}{\psi}_i {\, \mathrm{d}x}
        {\thinspace .}
...
.. math::
   :label: fem:deq:diffu:FE:vf:u
        ...
        \int_{\Omega} u{\psi}_i{\, \mathrm{d}x} =
...
.. math::
        ...
        M &= \{M_{i,j}\},\quad M_{i,j}=({\psi}_i,{\psi}_j),\quad i,j\in{\mathcal{I}_V},\\ 
        K &= \{K_{i,j}\},\quad K_{i,j}=(\nabla{\psi}_i,\nabla{\psi}_j),\quad i,j\in{\mathcal{I}_V},\\ 
        f &= \{(f(\boldsymbol{x},t_n),{\psi}_i)\}_{i\in{\mathcal{I}_V}},\\ 
        c &= \{c_i\}_{i\in{\mathcal{I}_V}},\\ 
        c_1 &= \{c_{1,i}\}_{i\in{\mathcal{I}_V}}
        {\thinspace .}
...
.. _Eq:fem:deq:diffu:anal:Ae:

.. math::
   :label: fem:deq:diffu:anal:Ae
...
introducing an expansion :math:`u(x)=U_0{\varphi}_0 + U_N{\varphi}_N + \sum_{j=0}^{N}
c_j{\varphi}_{\nu(j)}`, with :math:`N=N_n-2` and considering :math:`u` values at the
...
Exercise 31: Solve a 1D finite element problem by hand
------------------------------------------------------
