...
has constant coefficients and is homogeneous. The solution is then
:math:`u^n=CA^n`, where :math:`A` is some number to be determined
from the differential equation and :math:`C` is determined from the
initial condition (:math:`C=I`).
...
considerably simplified if we seek an :math:`A` on the form
