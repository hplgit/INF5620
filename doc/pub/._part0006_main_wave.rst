...
7. The use of index sets ``Ix`` and ``Iy`` in the scalar code cannot be
   ...
   C loops. We have therefore introduced ``Ix_start`` and ``Ix_end`` to
   represent the start and end of the values of index :math:`i` with
   similar variables for the :math:`j` index.
