...
The different formats require different video encoders (``-vcodec``) to
be installed: Flash applies ``flv``, WebM applies ``libvpx``, and MP4
applies ``libx64``.  Players like ``vlc``, ``mplayer``, ``gxine``, and ``totem``
...
**b)**
Import the ``solver`` function ``wave1D_u0_s.py`` into a new file
