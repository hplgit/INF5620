...
better interpreted if we compute the *energy of the system*.
...
by :math:`u'` and integrate from :math:`0` to :math:`T`:


.. math::
         \int_0^T u''u' dt + \int_0^T\omega^2 u u' dt = 0{\thinspace .}

(When our ODE comes from Newton's section law, this action means that
we compute the work done on the mechanical system in the time
interval :math:`[0,T]`.)
...
where we have introduced the *energy* :math:`E(t)`
...
as a sum of the kinetic energy
...
.. math::
   :label: vib:model1:energy:potential
        ...
        E_p(t) = \frac{1}{2}\omega^2u^2{\thinspace .}
...
Analytically, we have :math:`u(t)=I\cos\omega t`,
so we can easily check the evolution of the energy
...
We can use this value of the constant for verification, or we can simply
compute :math:`E(0)` from the initial conditions and check that :math:`E(t_n)=E(0)`
at all mesh points :math:`t_n`.
