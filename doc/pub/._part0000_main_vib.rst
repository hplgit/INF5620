...
To summarize the formulas, the initial conditions are


.. math::
   :label: vib:model2x2:EulerCromer:u0s2
...
.. math::
   :label: vib:model2x2:EulerCromer:v0s2
          
        v^{\frac{1}{2}} = \frac{1}{2}\Delta t\omega^2I {\thinspace .}
        
        

We then realize that for :math:`n=1,2,\ldots`, :math:`u^{n}` can be computed from
:eq:`vib:model2x2:EulerCromer:ueq1s2` and then :math:`v^{n+1/2}` from
:eq:`vib:model2x2:EulerCromer:veq1s2` using the recently computed value
:math:`u^{n}` on the right-hand side.

*NOT FINISHED.* .. implement the staggered scheme
...
An Euler-Cromer scheme for the generalized model
------------------------------------------------
...
The initial conditions can be derived as in
the section :ref:`vib:model2x2:staggered`. Assuming the general conditions
:math:`u(0)=I` and :math:`u'(0)=V`, we can repeat the derivation and arrive at
...
.. code-block:: python
        ...
        def linear():
            print '=== Testing linear exact solution ==='
            print "Initial conditions u(0)=%s, u'(0)=%s:" % \ 
                  (u_e(t).subs(t, 0), sm.diff(u_e(t), t).subs(t, 0))
            ...
            print 'residual step1:', residual_discrete_eq_step1(u_e)
            print 'residual:', residual_discrete_eq(u_e)
        
        if __name__ == '__main__':
            linear()

Fill in the various functions such that the calls in the ``linear``
function works.

*d)* The purpose now is to choose a quadratic function
:math:`u_{\small\mbox{e}} = bt^2 + ct + d` as exact solution. Extend the ``sympy``
code above with a function ``quadratic`` that computes the
right source term ``f`` and evaluates the residual
in the two types of finite difference equations.
...
*e)* Implement a ``solver`` function for computing the numerical
solution of this problem.

*f)* Write a nose test for checking that the quadratic solution
