...
when :math:`x_{j}` is a node in the mesh with global node number :math:`j`. This
result arises
...
Let :math:`x_{i}`, :math:`i\inI`, be the nodes in the mesh.
Collocation means


.. math::
        
        u(x_{i})=f(x_{i}),\quad i\inI{\thinspace .}
        

With :math:`u(x)\approx\sum_{j\inI}c_j{\psi}_j(x)` and :math:`{\psi}_j`
chosen as :math:`{\varphi}_j`, we get


.. math::
         \sum_{j\inI} c_j {\varphi}_j(x_{i}) = f(x_{i}),
...
:math:`f(x_i)`, :math:`i\inI`, where :math:`x_i` denotes a mesh point.
